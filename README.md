...
#### [메모리 단편화](./os/230314_메모리_단편화와_페이징)

- RAM에서 메모리의 공간이 조각나서, 가용 메모리가 충분하지만 할당이 불가능한 상태.

- 외부 단편화

  - 연속 메모리 할당 및 해제가 반복되며 중간중간 빈 메모리의 총합은 충분하지만, 각 메모리의 최대치가 작아 프로세스를 할당할 수 없는 상태.
  - 페이징으로 해결 가능
- 페이징
  - 가상 메모리 기법으로, 같은 크기의 페이지(논리 주소)와 프레임(물리 주소)로 분할하여 프로세스를 나눠 할당
  - 페이지 테이블로 페이지-프레임 매핑
- 내부 단편화

  - 프로세스 크기가 페이지 크기로 나누어떨어지지 않으면, 마지막 페이지에 빈 공간 발생
  - 세그멘테이션으로 해결 가능
- 세그멘테이션
  - 서로 크기가 다른 메모리 블록 할당
  - 외부 단편화 존재
...
- 메모리를 이동하여 여유 공간을 만드는 작업.
- 작업 단위로 프로세스를 할당하면 작업이 끊기지 않고 잘 수행될 수 있지만, 메모리 공간이 부족할 때 Compaction을 하면 오버헤드 발생, 비용이 크기 때문에 잘 사용하지 않는다.
